...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="dark flex h-screen bg-slate-950 font-sans text-slate-100 overflow-hidden relative selection:bg-indigo-500/30">
      ...
      <motion.aside 
        initial={{ x: -300 }}
        animate={{ x: 0 }}
        className={`relative z-20 flex flex-col w-72 h-full border-r border-slate-800 bg-slate-900/50 backdrop-blur-xl transition-all duration-300 ${!isSidebarOpen && '-ml-72'}`}
      >
        <div className="p-6 pt-10 border-b border-slate-800/50 flex items-center gap-3">
           <div className="h-10 w-10 bg-gradient-to-br from-indigo-600 to-blue-500 rounded-xl flex items-center justify-center text-white font-bold shadow-lg shadow-indigo-500/20">
             MS
           </div>
           <div>
        ...
        <nav className="px-4 py-8 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        <div className="mt-auto p-4 border-t border-slate-800/50 bg-slate-900/30">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative z-10 bg-slate-950/50">
        {/* Glass Header */}
        <header className="h-20 flex items-center justify-between px-8 border-b border-slate-800/60 bg-slate-900/80 backdrop-blur-xl sticky top-0 z-50">
           <div className="flex items-center gap-4">
              <button onClick={() => setSidebarOpen(!isSidebarOpen)} className="p-2 -ml-2 rounded-lg text-slate-400 hover:text-white hover:bg-white/5 transition">
                  <Menu size={20} />
              </button>
              <div className="relative group">
                  ...
                  <input 
                    ...
                    className="h-10 w-64 bg-slate-900/50 border border-slate-700/50 rounded-xl pl-10 pr-4 text-sm text-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-transparent placeholder:text-slate-600 transition-all"
                  />
              </div>
           </div>
        ...
        {/* Scrollable Content */}
        <main className="flex-1 overflow-auto p-8 custom-scrollbar scroll-smooth">
